...
//
// Host communicates via the Xilinx JTAG-to-AXI Master IP (same USB cable as
// programming).  No UART required.  Pixel data streams directly from JTAG-AXI
// to the encoder; a 64 KB on-chip JPEG buffer holds the compressed output.
//
...
//                               DEMO_STATUS (read)   [0]=enc_done  (same addr)
...
module demo_top #(
    ...
    // -----------------------------------------------------------------------
    // JTAG-to-AXI Master (Xilinx IP, generated by create_project.tcl)
    // -----------------------------------------------------------------------
    ...
    reg [31:0] ar_addr;
    reg [7:0]  ar_rem;
    reg [15:0] ar_widx;   // declared here; referenced in jpeg_rd_data block above
