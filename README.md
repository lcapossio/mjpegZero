...
[![RTL: VHDL-2008](https://img.shields.io/badge/RTL-VHDL--2008-blue.svg)](rtl/vhdl/)
...
VHDL-2008 port under `rtl/vhdl/`, AXI interfaces, and up to 1080p30 on low end AMD/Xilinx 7-Series FPGAs. Two operating modes: **Full** encodes with runtime quality control;
...
hardware test with identical 235,118-byte JPEG output and Y-PSNR 38.45 dB
versus the original image.

<a id="resource-usage"></a>
## Resource Usage <sub>[↑ Top](#top)</sub>

The board-level numbers below are for the full Arty A7-100T demo design,
including the MJPEG encoder, shared Arty wrapper, fcapz EJTAG-AXI debug bridge,
fcapz ELA, and the on-chip JPEG readback buffer used by the hardware demo.

Current hardware-verified configuration: Lite mode, 1280x720, Q75. The Arty
A7-100T demo has been built and hardware-tested with both the Verilog encoder
and the VHDL encoder.

| Configuration | HDL | LUTs | FFs | BRAM tiles | DSP48E1 | WNS |
|---------------|-----|-----:|----:|-----------:|--------:|----:|
| Arty A7-100T demo, Lite 720p Q75, 150 MHz | Verilog | 5,362 | 5,265 | 78 | 21 | +0.153 ns |
| Arty A7-100T demo, Lite 720p Q75, 150 MHz | VHDL | 5,348 | 5,258 | 78 | 21 | +0.167 ns |

These full demo numbers include the shared Arty wrapper, fcapz EJTAG-AXI debug
bridge, ELA, and the on-chip JPEG readback buffer. Core-only apples-to-apples
reports are generated by `scripts/synth/amd/run_core_synth.tcl`.
...
| SDP BRAM            | `rtl/bram_sdp.v`             | `rtl/vhdl/bram_sdp.vhd`             | Behavioural wrapper; vendor-specific primitives in `rtl/vendor/` |
...
The encoder is maintained in behavioural Verilog 2001 and native VHDL-2008.
The only vendor-specific RTL primitive is the simple dual-port BRAM wrapper
under `rtl/vendor/<vendor>/`. Everything else is portable and synchronous.
...
# Native VHDL-2008 encoder synthesis
...
mjpegZero/
  rtl/              Synthesizable Verilog 2001 source
    vhdl/           Native VHDL-2008 encoder sources
